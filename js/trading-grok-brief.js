(() => {
  ...
  const shell = document.querySelector('#grok-brief-shell');
  if (!shell) return;
  ...
  const tickers = (primary = [], secondary = []) => {
    ...
    const secondaryHtml = secondary.length
      ? ` <span class="grok-brief-secondary">read-through ${secondary.map(symbol => `<code class="brief-ticker">${esc(symbol)}</code>`).join(' ')}</span>`
      : '';
  ...
  function renderThesis(thesis) {
    ...
    const chain = (thesis.catalyst_chain || []).map((step, index) => (
      `<li><span class="grok-brief-step">${index + 1}</span>${esc(step)}</li>`
    )).join('');
    const watch = (thesis.watch || []).map(item => `<li>${esc(item)}</li>`).join('');
    const confidence = Number(thesis.confidence);
    return `<article class="brief-risk-card grok-brief-thesis-card" data-thesis-id="${esc(thesis.id)}" data-agency="${esc(thesis.agency)}" data-priority="${esc(thesis.priority)}" data-stage="${esc(thesis.narrative_stage)}">
      ...
      <div class="grok-brief-chain"><h4>Catalyst chain</h4><ol class="grok-brief-chain-list">${chain}</ol></div>
  ...
  fetch(shell.dataset.url, { credentials: 'same-origin' })
    .then(response => {
      if (!response.ok) throw new Error(`Grok brief fetch failed: ${response.status}`);
    ...
    .then(data => {
      ...
      shell.innerHTML = `
        <div class="position-head">
          <h2 id="grok-brief-heading">Grok Catalyst Brief</h2>
        ...
        <div class="brief-log" data-grok-brief-as-of="${esc(data.as_of)}">${data.theses.map(renderThesis).join('')}</div>
    ...
    .catch(error => {
      shell.innerHTML = `<div class="position-head"><h2>Grok Catalyst Brief</h2></div><p class="trading-note">Grok brief unavailable: ${esc(error.message)}</p>`;
      console.error(error);
    });
})();
