...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        check(position_toggles.evaluate_all("nodes => nodes.every(node => node.getAttribute('aria-expanded') === 'true')"), "live position setups are not expanded by default")
        desktop.wait_for_function("[...document.querySelectorAll('#bl-built [data-position-chart-detail]')].every(node => !node.hidden && node.querySelectorAll('svg').length === 2)")
