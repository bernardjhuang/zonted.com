...
def render_entry(entry: dict, latest: bool) -> str:
    ...
    return f'''<article class="autonomous-entry" id="entry-{esc(entry["id"])}" data-decision="{esc(entry["decision"])}">
      ...
      <section aria-labelledby="strategies-{esc(entry["id"])}"><h3 id="strategies-{esc(entry["id"])}">Strategies</h3><div class="autonomous-grid">{strategies}</div></section>
      ...
      <section aria-labelledby="reviews-{esc(entry["id"])}"><div class="autonomous-section-head"><h3 id="reviews-{esc(entry["id"])}">Adversarial review</h3><span>dual-review status · PASS</span></div><div class="autonomous-reviews">{approvals}</div>{bullets(entry["review_summary"]["changes_after_grok"], "autonomous-changes")}</section>
