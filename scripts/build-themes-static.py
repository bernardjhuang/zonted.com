...
def render_static(payload: dict, data_hash: str) -> str:
    ...
    if qualified:
        blocks.extend([
            ...
            "<h2>Active plays</h2>",
            f"<p>{len(qualified)} {'thesis' if len(qualified) == 1 else 'theses'} the desk has decided to play — research "
            f"graduated into live positions. Scores stay frozen at qualification; the position note records what was bought, "
            f"when, and the build plan. Everything below remains research.</p>",
        ])
    ...
    blocks.extend([
        ...
        f"{len(qualified)} active {'play sits' if len(qualified) == 1 else 'plays sit'} above as live desk positions. "
