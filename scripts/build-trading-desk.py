...
def render_status_metrics(page: str, positions: int, status: dict[str, float]) -> str:
    ...
    rendered, count = re.subn(
        pattern,
        lambda match: match.group(1) + "\n    " + sector_chips + metrics + match.group(2),
        page,
        count=1,
        flags=re.S,
    )
...
def sync_shell_assets(stamp: str | None = None, status_metrics: str | None = None) -> None:
    ...
    if status_metrics:
        for retired in ("gpt", "grok", "fable", "gemini", "meta"):
            status_metrics = re.sub(
                rf'<a class="chip chip-{retired} [^"]+" href="/trading/(?:gpt|grok|fable|gemini|meta)-risk/".*?</a>',
                "",
                status_metrics,
                count=1,
            )
