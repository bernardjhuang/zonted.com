...
class RoutedTradingSyncTests(unittest.TestCase):
    ...
    def test_meta_risk_route_preserves_the_search_grounded_model_output(self) -> None:
        ...
        self.assertEqual(entry["derived_rating"], 4)
    ...
    def test_trading_nav_has_home_logo_and_vwap_uses_three_columns(self) -> None:
        ...
        for path in pages:
            ...
            self.assertIn('<a class="chip chip-gpt chip-on" href="/trading/gpt-risk/" title="GPT risk appetite — Neutral · 5.5/10">GPT 5.5</a>', page, path.as_posix())
            self.assertIn('<a class="chip chip-grok chip-on" href="/trading/grok-risk/" title="Grok risk appetite — Mildly risk-on / neutral-constructive · 6/10">Grok 6</a>', page, path.as_posix())
            self.assertRegex(page, r'class="chip chip-gemini [^"]+" href="/trading/gemini-risk/"')
            self.assertRegex(page, r'class="chip chip-meta [^"]+" href="/trading/meta-risk/"')
