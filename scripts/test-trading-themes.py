...
class TradingThemesContractTest(unittest.TestCase):
    ...
    def test_energy_theme_has_final_adversarial_synthesis(self) -> None:
        ...
        self.assertGreaterEqual(len(self.payload["themes"]), 65)
    ...
    def test_disqualified_theme_is_retained_but_separated_from_core_ledger(self) -> None:
        ...
        self.assertIn("themes.filter(theme => !theme.disqualified && !theme.qualified)", self.script)
        self.assertIn("themes.filter(theme => theme.qualified && !theme.disqualified)", self.script)
    ...
    def test_meta_frontier_themes_are_complete_and_independently_scored(self) -> None:
        expected = {
            "meta-power-wall-rate-shock": ("Energy", 68, 32, 80, 50),
            "meta-humanoid-labor-wage-shock": ("Emerging", 38, 14, 62, 40),
